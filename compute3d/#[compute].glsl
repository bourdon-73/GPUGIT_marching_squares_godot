...
void main()
{
	...
	//matrix.data[index] = id.xy * 32;
	matrix.data[index] = vec4(evaluate(vec3(id.x + 0, id.y + 0, id.z + 0)));
	...
	for (int i = 0; i < numIndices; i += 3) {
		...
		int v0 = lut.data[offset + i]; //0
		int v1 = lut.data[offset + 1 + i];//8
		int v2 = lut.data[offset + 2 + i];//3

		int a0 = cornerIndexAFromEdge[v0];//0
		int b0 = cornerIndexBFromEdge[v0];//0

		int a1 = cornerIndexAFromEdge[v1]; //cornerIndexAFromEdge[8] 
		int b1 = cornerIndexBFromEdge[v1];//8

		int a2 = cornerIndexAFromEdge[v2];//3
		int b2 = cornerIndexBFromEdge[v2];//3
